...
composed *after* the existing extension list; **no name-based override of the
measurement** (Q7 — SN-003's acceptance extensions are examples, not rulings); and a
**per-set probe mode** `CompressProbe: Off | ExcludedExtensions |
...
finding is folded, and the one open item it produced was **Q7**. **Revision 4 (2026-09-01):** the
Owner ruled Q7 — the SN-003 acceptance line is an *example*; the need is *"optionally
compress stored data to save more space"*, and a measured decision fulfils it better than
any extension — so the ruled list is **removed** in every direction and the probe decides
for every name. Registry ids in this document are
...
compresses. There is **no name-based override** of the measurement: SN-003's acceptance
line names `.docx`/`.txt` and `.jpg`/`.mp4` as *examples* of the need (Owner ruling, Q7),
and the need — save space — is served by the bytes, not the name; the extension list
does not disappear, it serves the two non-default modes and the below-floor tail. The
probe runs **lazily** — only when the group actually has to write, never
...
| **I-2** | **SN-003's *need* is what WP17 serves; its acceptance line's extensions are examples, not rulings** (Owner, Q7, 2026-09-01). The need reads *"Optionally compress stored data to save more space"*; the acceptance line — *"a `.docx`/`.txt` is stored as `.7z`; a `.jpg`/`.mp4` is stored as-is"* — illustrates it for the pre-WP17 mechanism. | No name-based override exists in any mode: a `.docx` over incompressible bytes is stored raw, a `.jpg` over compressible bytes is stored `.7z`, and both save more space than the example would. The `Common.psm1:112` comment that calls the office-format exclusion "a stakeholder decision, not an oversight" is rewritten in Part A to record the Q7 ruling instead, and TC-096's SN-003 arm is re-scoped (§2.3). SN-003's text is **not** edited (Q5); §2.3 records the one consequence of leaving it — the release checklist quotes acceptance lines verbatim. TC-224 pins the ruling. |
...
- **`Resolve-CompressionDecision`** (pure, no I/O). One signature, no ambiguity:
  ...
  $true` so it carries only the list's opinion), `Length`, `ProbeMinBytes`,
  `SampledBytes`, `CompressedBytes`, `Threshold`. Output `{ Compress; Reason }` with
  `Reason ∈ {CompressDisabled, ModeOff, BelowFloor, ListExempt, ProbeUnavailable,
  ProbeCompressible, ProbeIncompressible}`. Rule, in order:
  0. `CompressEnabled = false` → raw, `CompressDisabled`. **Nothing below runs.** (Both
     ...
     that consulted the probe first would name a `.7z` object and
  ...
  1. *(removed — Q7; no name-based override.)*
...
### 2.3 No ruled list — SN-003's examples are examples (Q7)

Revisions 1–3 carried an extension list the measurement could not overrule, built from
SN-003's acceptance line: first `.docx`-family → compress, then (after the cross-review's
R-2) a directional list adding `.jpg`/`.mp4` → raw. **The Owner removed it (Q7,
2026-09-01):** the need is *"Optionally compress stored data to save more space"*; the
acceptance line's extensions were an illustration of the extension mechanism, not a
ruling on those formats; and a probe that stores a `.docx` raw when its bytes will not
shrink, or compresses a `.jpg` whose bytes will, fulfils the need *better* than any
name. So there is nothing in Engine named "ruled", and `Resolve-CompressionDecision`
has no such input. Under `always` every file above the floor is measured, without
exception.

Three consequences are recorded so nobody re-derives the old list from the artifacts
that still mention it:

- **`Common.psm1:110-114`** says office/text formats are excluded from the list because
  *"SN-003's acceptance line says a `.docx` / `.txt` IS stored as `.7z`, which is a
  stakeholder decision, not an oversight."* Part A rewrites that comment: the exclusion
  stands (the list is for formats known to be compressed, and office containers are
  not on it), but its justification is now *"SN-003's line is an example; under WP17 the
  bytes decide"*. This edit rides the same Common commit as the list, so it costs no
  extra KitRevision.
- **TC-096's "SN-003 acceptance-line extensions" arm** asserts `.docx .txt .xlsx .csv
  .log .bin` compress *because SN-003 says so*. It is re-scoped to what it can honestly
  claim: `Test-ShouldCompress` (the list) says compress for those extensions. The
  composed decision's behaviour on them is TC-224's.
- **The release checklist** (`scripts/gen_release_checklist.py`) quotes SN acceptance
  lines verbatim, so it will ask a human to confirm *"a `.docx`/`.txt` is stored as
  `.7z`"* — which, for a `.docx` above the floor whose bytes do not shrink, is false
  under `always`. SN-003 is not edited (Q5), so the checklist entry is answered with
  a pointer to this section. If that ever grates, the one-word hedge *"e.g."* in the
  acceptance line fixes it without naming the mechanism; the Owner declined it for now.
...
`CompressEnabled=false` precedes all three (rule 0). Any other value — mixed case included — is refused with the SR-042 exit 2
...
(3) that no name overrides the measurement, and why (Q7); (4) the three `CompressProbe` modes and the first-run probe-read cost; (5) the unchanged
...
| id | change |
|---|---|
| **SN-003** | *Unchanged — Owner rulings Q5 and Q7.* Its acceptance line is an example of the need, not a constraint on WP17 (I-2, §2.3); the release-checklist consequence is recorded in §2.3. |
| **SR-004** | *Amended.* "...except for already-compressed extensions" becomes "...except for (a) already-compressed extensions, in mode `off`/`excluded-extensions` and below the floor in `always`, and (b) content the SR-081 probe measures as incompressible." The acceptance clause *"an already-compressed extension is stored verbatim and Compressed=false"* is **re-scoped the same way** — under `always` above the floor TC-225 deliberately asserts the opposite. The name/column agreement sentence stays verbatim. Permutations: keep `compress=set{on,off}`; add `mode=set{off,excluded-extensions,always}; decision=set{compress-disabled,list-exempt,below-floor,probe-compressible,probe-incompressible,probe-unavailable}`. |
| **SR-081** *(new, `probe-v1`)* | **Measured compressibility gate.** Rule 0 (`CompressEnabled` dominates), the ordered rule, the aggregate threshold, the floor, the list-answer fallback (I-6), the absence of any name-based override (I-2/Q7), lazy memoized evaluation only at a group write walking the group's candidates (§2.4), the three-valued `CompressProbe` with `always` default, the log line and counters, and the explicit non-claims: no re-forming, no restore involvement, no new exit code. Refs `SN-003`, `SN-011`. Verification: `Test`. |
...
| **LLR-086** *(new, `probe-v1`)* | Engine: `Measure-SampleCompressibility`, `Resolve-CompressionDecision`, the two constants, the memoized call site and candidate walk in `Invoke-BackupFileGroup`, the `-CompressProbe` parameter and step-10 plumbing, the log/summary emission, the calibration table and the codec it chose. Helpers are module-internal, tested via `InModuleScope`; if exported, the `Export-ModuleMember` list (`:6590`) and the generated module map move in the same commit. |
...
`Common.psm1` list edit **and the `:110-114` comment rewrite (§2.3)**; README table;
TC-096 parameter block and the re-scoped SN-003 arm (§2.3); LLR-004 detail; both `KitRevision` markers `current+1`,
...
The two functions, the constants, comment-based help +
...
over the full `CompressEnabled × mode × list × floor × ratio` grid, rule 0
...
```
compress=set{on,off}; mode=set{off,excluded-extensions,always}; content=set{random,text,mixed-head,mixed-third,small,empty}; ext=set{listed,sn003-example,unlisted,none}; size=set{below-floor,above-floor}; dedup=set{fresh,existing-hit,in-run-twin,failed-then-twin}; format=set{json,clixml}
```
...
| **TC-224** | **Q7 pin (I-2):** under `always`, a `.docx` over random bytes above the floor is stored **raw**; a `.jpg` over text bytes is stored **`.7z`**; a `.txt` is stored `.7z`; and under `off` the same three take today's list answer (`.7z`, raw, `.7z`). No `Reason` value named after a format exists. | Pins the Owner's ruling that the bytes, not SN-003's example names, decide — and that `off` still reproduces the example. |
...
| **TC-231** | **Compression disabled dominates (rule 0):** with `CompressEnabled=false`, every mode × every extension class (SN-003 example names, listed, unlisted, none) stores raw with `Compressed=No`, and the probe is never called; and a Part-A end-to-end arm: a `.z7` source under `CompressEnabled=true`, mode `off`, is stored raw and `7z` is not invoked (mocked `Compress-FileWithSevenZip` counter = 0). | The reviewers' P0, and the hub's actual fix path. |

Negative controls that must be shown to fail fix-removed: **TC-225** (probe bypassed), **TC-226** (memo removed), **TC-231** (rule 0
removed).
...
|---|---|---|
| **Q0** | Scope | **A + B + mode**, as proposed (the ruled list later removed by Q7). Part A shippable alone (§8 row 3). |
...
| **Q7** *(from the cross-review; ruled 2026-09-01)* | Does `always` withhold **`.jpg` and `.mp4`** from the probe so SN-003's acceptance clause stays literally true? | **No.** *"The acceptance intent is an example; the need is literally 'Optionally compress stored data to save more space'; the probe may still compress `.jpg` and `.mp4` — that fulfils the need better than a file-extension restriction."* Applied consistently to the `.docx`/`.txt` half as well: the ruled list is removed in both directions (§2.3, I-2). The driver's recommendation ("yes") is superseded. |
...
| **Measure-then-override for telemetry honesty** | Codex #3: under `always`, probe overridden files anyway and log the ratio. Moot after Q7 (no override exists); kept for the record. |
...
**Owner ruling (2026-09-01): the Owner removes the residue by hand; it is outside
WP17's scope and was context for the design only.** The procedure below stays as the
record of what "remove" should mean: since no run has ever completed against this store, the backup
...
| R-2 | ★ Codex 2 (P0), Opus 11 (P1) | **P0** | The ruled model protects only SN-003's `.docx` half; a boolean override cannot force `.jpg`/`.mp4` raw, and the plan's claim that the clause "holds because those bytes probe incompressible" is empirical, not invariant. | **A, then superseded by Q7** — revision 3 answered with a directional ruled list; the Owner's Q7 ruling (2026-09-01) removed the list in both directions instead: the acceptance line is an example, the need is space, the bytes decide (§2.3, I-2). The finding's substance — that "those bytes probe incompressible" was never an invariant — stands and is why the plan no longer claims it. |
| R-3 | Codex 3 | P1 | Under `always`, measure ruled files anyway and apply the ruled outcome afterwards, so "probes all files" and the telemetry stay literally true. | **Moot after Q7** — with no ruled list, `always` measures everything above the floor and the contract is literally true. (Before Q7 it was rejected as a read spent on a decided outcome.) §7 row retained for the record. |
...
## 11. Implementation review round, 2026-09-01 — composite disposition (after commit 5)

The §8 independent review: two adversarial passes over the implemented diff
`41a201f..03e628e` (registry + Parts A, B1, B2), run in parallel with the same brief
(nine attack surfaces) and no implementation context — an **OpenAI Codex CLI** agent
(`codex exec --sandbox read-only`; 1 P0, 5 P1, 3 P2) and a **fresh-context Claude Opus**
reviewer (0 P0, 1 P1, 8 P2). The coordinator verified every finding against the tree — and
reproduced R-1, R-3 and R-4 live — before disposing of it. Findings raised **independently by
both** are marked ★ and were folded first. `A` = accepted and folded; `A′` = accepted in
modified form; `R` = rejected, reason given. Fixes land as **commit 5.1**, then row 5's gate
(`-Gate G2 -Tier Full`) re-runs.

| # | Raised by | Sev | Finding | Disposition |
|---|---|---|---|---|
| R-1 | ★ Codex 1 (P0), Opus 1 (P1) | **P0/P1** | The new non-`Deep` Verify exemption (`Get-StorageFormFinding`) fires for ANY `Compressed=No` archive-magic object whose DataPath extension and on-disk length match the row — so a raw-stored `photo.jpg` whose 4096 bytes are replaced by a different 4096-byte payload carrying 7-Zip magic is no longer hashed and `FlagOverArchive` (repairable) is no longer raised non-`Deep`. Opus reproduced both sides against `41a201f` and HEAD. TC-229's negative arms cover only length- and extension-mismatch. The code comment's "payload identity is unweakened" over-claims. | **A′** — the cheap path is **narrowed to the shape Part A ships to fix**: the row's RelativePath extension must ALSO be one the extension list exempts (`Test-ShouldCompress -CompressEnabled $true` is `$false`) — the hub's 649 `.z7` (and every `.7z`/`.zip`/… stored raw) keep the 880 GB saving, while an archive-magic object under a non-listed name keeps the confirming hash exactly as before WP17. The comment and LLR-049 are reworded to say what the non-`Deep` scan does and does not prove (`-Deep` is the payload audit; the non-`Deep` scan never hashed any other raw object). TC-229 gains two arms: unlisted-extension archive-magic same-length ⇒ hashed and flagged; listed-extension same-length different payload ⇒ exempt non-`Deep`, caught under `-Deep` (the reduced assurance stated in a test, not hidden). Codex's "retain the hash for all" is **R**: it re-creates the per-Verify re-hash of the library for exactly the class Part A exists to fix, and the Owner approved the exemption (plan §2.1). Narrowing is the conservative direction; **surfaced to the Owner in status.md** (dial HIGH). Engine still holds no copy of the list (I-4: it calls `Test-ShouldCompress`, as it already does). |
| R-2 | Codex 2 | P1 | An unchanged v2 config changes meaning (absent key ⇒ `always`, a large compressible `.jpg` now compresses); bump `ConfigVersion` to 3 or default absent to legacy. | **R** — re-litigates §10 R-16 and the Owner's Q4 ruling: `always` is the ruled default, SR-063 ratifies the additive-optional policy, and a version bump would force every deployment (HomeHub) to edit a config for a key it need not set. Recorded; nothing folded. |
| R-3 | Codex 3 | P1 | CLIXML accepts `CompressProbe = [object[]]@('off')`: the `[string]` cast joins a one-element array to `"off"`, so JSON and CLIXML do not enforce the same scalar contract. **Reproduced by the coordinator — and the `BrowseView` precedent has the identical hole.** | **A** — `Test-BackupConfigurationShape` requires the value (its `BaseObject` when PSObject-wrapped) to be a scalar `[string]` before the `-cnotin` check, for both formats; CLIXML fixtures/arms for one- and multi-element arrays added to TC-227. `BrowseView`'s identical hole is **out of WP17's scope** and is surfaced to the Owner in status.md as a follow-up (the fix is the same two lines). |
| R-4 | ★ Codex 4 (P1), Opus 9 (P2) | P1/P2 | The `FileShare.Read` rationale ("no window can sample a file being rewritten underneath it") is false: a writer already holding the file lets the probe read changing bytes; on POSIX .NET's sharing is advisory. Codex proposes a size/mtime/identity check before and after sampling. | **A′** — **On Windows the sentence is TRUE**, verified live by the coordinator: with a writer holding the file (`FileAccess.ReadWrite`, `FileShare.ReadWrite`) a `FileShare.Read` open fails with a sharing violation (the share mode requested excludes writers; Windows checks both directions). Opus's premise is wrong for Windows. **On Linux** .NET's `FileShare` emulation is advisory, so the claim is qualified by platform in the comment, SR-081 and LLR-086. The cheap stability check is folded: length and `LastWriteTimeUtc` are read before and after the sample and a change returns `$null` (⇒ the list's answer). File-identity comparison **R** — no portable API is needed for a decision that never touches data. The check has no deterministic unit arm (it needs a concurrent writer); it lives inside the `try`, so its worst case is `$null`. |
| R-5 | ★ Codex 5 (P1), Opus 8 (P2) | P1/P2 | TC-229's full arm (probe-mixed store, both restorers) and TC-230 (container) are absent; check.ps1's Release tier invokes no container step; no G-suite fixture exceeds the 256 KiB floor, so the G1–G9 matrix runs the probe path zero times. | **R as premature, A′ on substance** — the arms are §8 rows 7 and 8 by the approved sequencing (the WP14 R-11/R-12 pattern). Substance folded: (i) row 7's TC-229 store is built from an above-floor compressible file, an above-floor incompressible file and a `.z7`, restored by `Reconstruct.ps1` AND `reconstruct.sh`; (ii) TC-230's mechanism is `scripts/Invoke-Container.ps1 -Action BuildAndTest` (the `container-v1` evidence path — check.ps1's Release tier was never the container runner), recorded at row 8; (iii) the G-suite observation is recorded — it also proves the `always` default moved no existing suite's expectation — and no G-suite fixture is added now (suite runtime); TC-229/TC-230 carry the above-floor coverage. |
| R-6 | Codex 6 | P1 | TC-077 proves schema↔validator parity only over hand-picked fixtures; adding `"auto"` to the schema enum alone is undetected. | **A′** — TC-227 gains a vocabulary-parity arm: the schema's `CompressProbe` `enum` must equal exactly `off, excluded-extensions, always` (case-sensitive) and equal the `ValidateSet` on `Invoke-BackupFileGroup -CompressProbe` and `Resolve-CompressionDecision -Mode` (read from command metadata). Generating both surfaces from one constant is **R** (the schema is a static published file). |
| R-7 | Codex 7 | P2 | Middle-window offset is computed in `double`; above 2^53 (files > 9 PB) it can be off by one byte. | **R** — no supported filesystem holds such a file in practice, the displaced window is still a valid 256 KiB sample of the same file, and both outcomes are correct forms (I-3). Recorded. |
| R-8 | ★ Codex 8 (P2), Opus (TC-228 remark) | P2 | TC-228's registry text demands a `FileShare.None` hold on every member AND exit 0 — impossible together, since the copy cannot read a locked file either. The test implements two honest arms. | **A** — TC-228's row is split into the two arms implemented: (A) probe unavailable with readable bytes ⇒ list's answer, one WARN, write succeeds; (B) real exclusive holds ⇒ `ProbeUnavailable`, one WARN, no probe exception, and only SR-067's ordinary copy failure. |
| R-9 | Codex 9 | P2 | README still describes the extension-only rule under the new default. | **R as premature** — Part C (row 7, §2.6) rewrites that section; folded there, sub-heading preserved for TC-096. |
| R-10 | Opus 2 | P2 | A hand-built set with `CompressProbe = 'Always'` passes three case-insensitive `[ValidateSet]`s, the probe READS the bytes, then `Resolve-CompressionDecision`'s `-cnotin` throws — mid-loop, after partial writes, naming a private helper. Reproduced. | **A** — `Invoke-BackupSet` refuses a non-lowercase mode with the SR-042 wording BEFORE the group loop (nothing read, nothing written), and `Resolve-GroupStorageForm` checks `-cnotin` before any probe. Direct arm: `Invoke-BackupFileGroup -CompressProbe 'Always'` throws with zero probe calls. |
| R-11 | Opus 3 | P2 | TC-224's transcript assertion indexes token 3 of the log line — the literal `compress-decision:` — so it can never fail; the Reason is token 5. Verified. | **A** — token 5. |
| R-12 | Opus 4 | P2 | TC-226's "at most one WARN" passes on 0 because the mocked probe never yields `ProbeUnavailable`; the memo-WARN property is unpinned there. | **A′** — the property IS pinned by TC-228 arm B (the memo-removed negative control flipped it: "Expected 1, but got 2"); TC-226 gains the honest arm anyway: probe `$null` + first member's copy fails + twin succeeds ⇒ exactly one WARN. |
| R-13 | Opus 5 | P2 | `Resolve-GroupStorageForm`, `New-ProbeStatistic`, `Write-ProbeStatistic` carry `Implements: SR-081, LLR-086` but LLR-086's CodeSymbol lists three names; LLR-058 lists only `Invoke-BackupFileGroup`. | **A** — both CodeSymbol cells amended. |
| R-14 | Opus 6 | P2 | The three counters count only probe-decided objects, so a `CompressProbe: off` run reports zeros; rename or add a partition counter. | **R** — the lines are the plan's (§2.4), the `Probe` prefix scopes them to probe decisions by design, and `Probe reads` is the cost figure they exist for; a run-wide partition of stored forms is WP16's observability domain. Recorded. |
| R-15 | Opus 7 | P2 | After a list or mode change the pool can hold BOTH forms of one `(hash,length)` (a `.z7` archived pre-Part-A as `.7z`, re-added post-Part-A as raw `.z7`); `Optimize-ChangeFolders` keeps the backup copy and blanks the snapshot row, whose `Compressed=Yes` then disagrees with the surviving raw copy ⇒ a non-repairable `BlankRowFormDisagreement` on a healthy store. Restore unaffected (both restorers decide form from the located file). | **A′** — a documentation fold: LLR-049 and README's mixed-form paragraph (row 7) name this as an expected, benign finding after a list or mode change. The audit-semantics change (flag only when NO pool location agrees) is a second edit inside the data-integrity audit outside the approved plan and is **surfaced to the Owner** as an optional follow-up rather than made here. The class predates WP17 (WP5's eight additions could produce it). |
| R-16 | both (verified-sound blocks) | — | Write branch: `$ownerCompress` written once from the memo and read only after, inside the `else`; dedup/twin branches return first; memo survives `continue`; two members cannot take different forms; a 7-Zip failure writes no row. Rule order vs `probeDecides` cannot disagree; `SampledBytes ≤ 0` is unreachable for a real probe; geometry at `3S`, `3S+5`, `512000`, `262144`; `Windows` is `Double[]` in both shapes; random measures 1.00002 (count after Dispose); boundary 900/901; `-Deep` always hashes; nine config surfaces present, JSON/CLIXML null/number/whitespace/case parity; I-4/I-5/I-8; SR-052 sizes by uncompressed length; counters per set and reference-mutated; lint zero findings; every Unit file runs at every tier; the 72-cell and 42-cell grids complete; TC-231 is a live negative control. | Recorded as the checked surface. |

**Net effect:** one finding both reviewers raised (R-1) narrows the Verify exemption to the
list-exempt shape — more conservative than the approved rule, same hub saving; one real
validator hole (R-3) and one real boundary hole (R-10) closed; one false premise corrected
(R-4: true on Windows, advisory on POSIX); three vacuous or over-claiming test assertions
fixed (R-11, R-12, R-8); registry cells trued up (R-13). Two findings re-litigated
Owner-ruled decisions and were rejected (R-2, R-14); two were premature for rows 7–8 (R-5,
R-9) and their substance is scheduled there. Three items are **surfaced to the Owner** in
status.md without blocking: the R-1 narrowing itself, `BrowseView`'s identical array hole
(R-3), and the blank-row audit semantics (R-15).
