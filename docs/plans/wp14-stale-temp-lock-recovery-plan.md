...
## 11. Implementation review round, 2026-08-31 — composite disposition (after commit 5)

The §8 independent review: two adversarial passes over the implemented diff
`92ab005..db72a19` (registry + Parts A–C), run in parallel with the same brief and no
implementation context — an OpenAI Codex CLI agent (read-only, ~207 k tokens) and a
fresh-context Claude Opus reviewer. Two findings were raised independently by both;
the Opus reviewer reproduced its P0 live. The coordinator verified every finding
against the tree before disposition. Fixes land as commit 5.1 before Part D.

| # | finding (both = raised independently twice) | sev | disposition |
|---|---|---|---|
| R-1 | The four pre-fence `catch` cleanups `Remove-Item -Recurse` a Temp the run may no longer own: a frozen-then-reclaimed owner that resumes and throws an ORDINARY error (source walk, manifest sanitation) recursively deletes the SUCCESSOR's staging folder — including evicted pool bytes whose only copy is there. Reproduced live by the Opus reviewer. The in-code "lexically before the first fence" argument covers lock-LOST throws, not ordinary throws after a silent loss. (both) | P0 | **Accepted.** The four cleanups are replaced by one helper that proves ownership first (marker present, RunId ours), then deletes only the exact marker leaf plus a non-recursive `Directory.Delete` — the same I-1 discipline as the reclaim path; any mismatch, foreign content, or I/O doubt keeps the folder and logs `[SR-075/lock-lost]`. `Remove-Item -Recurse` leaves the staging-cleanup surface entirely. New test: resurrection + early ordinary throw with a successor in place, successor's tree hash-identical after. |
| R-2 | The authoritative state is published UNFENCED: the backup-root `Write-Manifest` (and sidecar) sits between fences 2 and 3, so a run that lost its lock mid-phase overwrites the successor's live MANIFEST.csv before the finalize fence trips — TC-204 even skipped the manifest assertion for that fence. The step-7 staging manifest write is likewise before fence 1. (both; Codex rated P0) | P0 | **Accepted.** Fences added immediately before the step-7 staging write and immediately before the backup-root publish (the §2.5 set becomes five); the finalize fence stays. TC-204 extended to assert manifest, witness, sidecar, snapshot set and state file unchanged at every fence, per fence. |
| R-3 | Two reclaimers can BOTH succeed: the `Directory.Move` is not bound to the directory that was classified — a reclaimer stalled between its confirmation sample and its move can move the FIRST reclaimer's freshly recreated live Temp aside, then delete its marker-only aside. Winner-takes-all only holds when both movers target the same directory. (Codex) | P0 | **Accepted, with a different fix than proposed.** After a successful move, the aside folder is re-checked against the SAMPLED identity (marker RunId + evidence signature); a mismatch means a live directory was moved — it is moved straight back and the reclaim refuses. Failure to move back keeps the aside and refuses loudly; the displaced owner's worst case is a transient safe abort at its next fence/beat, never data loss. Codex's exclusive reclaim-claim file was **rejected**: a second filesystem lock artifact brings its own staleness and wedge modes and needs its own recovery protocol — the verify-after-move stays inside the existing primitive set and fails only toward refusal. Deterministic interleaving test added. |
| R-4 | The exclusive marker publish rests on non-overwriting `File.Move`, which on Unix is check-then-rename — NOT exclusive — and its atomic variant needs hard links exFAT does not have. Either every publish on the hub throws, or the theft-window closure is fiction on the exact deployment WP14 exists to fix. The temp-file staging half also writes into a folder the run may not own, and a crash mid-publish leaves a `.tmp` that classifies as `HoldsContent` — a NEW permanent wedge (Opus; Codex adjacent via the §10 rename history) | P1 | **Accepted.** The publish becomes a direct `FileMode.CreateNew` write of the final marker — exclusive by construction on every platform and filesystem, no temp file, no move. The §2.1 torn-read concern this reopens is already handled by design: a partially written marker is `ParseInvalid` with a FRESH mtime, which classifies `OwnerLive` and refuses — the safe direction. §2.1's publish mechanism is amended accordingly; SR-075/LLR-080 registry text amended in the same commit. TC-197 gains a successor-Temp enumeration assertion. |
| R-5 | Nothing ever verifies a single heartbeat lands: a store that rejects `SetLastWriteTimeUtc` (ACL write-data-not-attributes, bind-mount options) beats into `ErrorCount` that nobody reads, the mtime never moves, and after 1800 s a live run is classified dead — the confirmation sample then CONFIRMS it, because it detects change and nothing changes. (Opus) | P1 | **Accepted.** `Start-StagingHeartbeat` performs one synchronous beat and throws if the touch did not land — a store that cannot heartbeat must not be backed up under a lock that will read as dead; `Stop-StagingHeartbeat` logs `ErrorCount`/`LastError` when nonzero so a beat that dies mid-run is visible. |
| R-6 | The ~90 s confirmation sample ignores the `HeartbeatIntervalSeconds` the record committed to: a future writer with a legitimate 120 s cadence and a behind clock shows no movement inside a 90 s window and is reclaimed alive. (Codex) | P1 | **Accepted.** For a `Parsed` record the wait becomes `max(90, declared interval + 30 s margin)`, with the declared value validated and capped (600 s) so a hostile record cannot stall the reclaim path indefinitely. Test with a declared cadence longer than 90 s added. |
| R-7 | Heartbeat identity check has a read-to-touch TOCTOU: the callback closes the marker, then touches BY PATH — a reclaim landing between the two freshens the successor's marker once, delaying a later legitimate reclaim by up to one threshold. (Codex) | P2 | **Accepted** — cheap and closes the window exactly: the callback keeps the handle open and touches through `File.SetLastWriteTimeUtc(SafeFileHandle, …)`, so the touch lands on the same file object that was identity-checked. Bounded harm (one stray touch, no data path) is why this is P2, but the fix is smaller than the argument for skipping it. |
| R-8 | A fence read that fails transiently (`Unreadable` — one SMB/bind-mount blip) aborts the run mid-phase and leaves a content-holding Temp for the next run to refuse: WP14 converts a network hiccup into the class of wedge it was written to remove, three times per run. (Opus) | P2 | **Accepted.** The fence retries the marker read up to 3 times over ~1 s before treating `Unreadable` as lock-lost. Fail-closed is preserved — a genuine mismatch still aborts on the first read; only transient unreadability is absorbed. |
| R-9 | A non-already-exists `New-Item` failure (ENOSPC, EACCES, path length) is routed into the reclaim classifier, which reports "Temp already exists" — false — and discards the real exception. (Opus) | P2 | **Accepted.** The create failure is logged verbatim; the reclaim path is entered only when Temp actually exists as a directory; anything else rethrows the original error. |
| R-10 | Negative-control gaps: TC-204's control disables only the finalize fence; the Add-Type "module reload" test never reloads the module; TC-191's control is sequential rather than racing. (both, partially) | P2 | **Accepted in part.** TC-204's control now disables each fence point in turn; the reload test performs a real `Remove-Module`/`Import-Module -Force` in a child pwsh process; TC-191 gains the R-3 deterministic interleaving. **Rejected in part:** the demand to remove the drain and identity guard as separate mutations — they are compiled code, a shippable mutant class would be test-only theater, and the drain is already independently timed (Stop measurably blocks on a parked callback) while the same-RunId control isolates the identity comparison. |
| R-11 | Part D README/N-1/N-2/N-3 absent. (Codex) | P2 | **Rejected as premature** — Part D is §8 commit 6, deliberately sequenced after this review; N-1's verbatim README heading already ships in the Part C refusal text. Nothing to fold. |
| R-12 | TC-194/TC-195 exFAT/container evidence absent while reclaim is enabled unconditionally; Q1 unproven. (Codex) | P1→scheduled | **Rejected as premature in the same sense** — they are §8 commit 7, the package's own gate on `-Tier Release`, and the Q1 provider ruling is defined to come from that evidence. One substantive half is folded into R-4: the publish primitive no longer depends on rename semantics at all, which removes the worst provider-dependence before the evidence runs. The WP does not close, and SR-017/SR-075 do not flip to Verified, until commit 7's evidence is pasted. |
| R-13 | Areas independently attacked and found sound by the Opus pass (exFAT mtime granularity vs the 60/1800 s design, clock-skew both directions, drain ordering, non-recursive step-5 delete, two-reclaimer move-gate *as far as same-directory interleavings go*, Add-Type reload guard, case sensitivity, culture/encoding, literal paths, I-4/I-5) and by Codex (I-4/I-5). | — | Recorded so the next reviewer knows where effort went; R-3 shows the "sound" verdict on the two-reclaimer race was bounded by the interleavings traced. |
