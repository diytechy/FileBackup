...
| Exit-code table *(extended by WP1, 2026-08-23)* | one table shared with `Reconstruct.ps1`: **0** complete · **1** incomplete/content · **2** usage or precondition · **3** manifest-witness verification failed · **4** incomplete/host (retriable); precedence **2 > 3 > 4 > 1**. Normative copy: README "Restore exit codes". | SR-040 |
| Manifest witness *(added by WP1, 2026-08-23)* | verify `MANIFEST.csv` against its `MANIFEST.csv.meta` (`Bytes`/`Rows`/`XxH128`) **before** writing anything; absent sidecar = `UNVERIFIED` warning and continue (legacy backups), or abort under `--require-witness` | SR-038/SR-039 |
