...
| **-RepairFromPruned** (deferred from WP4 §5.4) | **RETIRED 2026-08-26 (human), premise expired - not built, and no longer wanted.** The question that killed it: *where would the bytes come from, if they are no longer in the source?* Three findings answer it. (1) A `Pruning_*` residue holds nothing anyone needs - re-homing and manifest publication both COMPLETE before the rename that commits the deletion, which is why `Remove-CommittedPruneResidue` can say "the pool was proven redundant before the rename". So there is nothing to salvage from a partially removed snapshot, and describing it that way (driver, 2026-08-26) was wrong. (2) Under content addressing a row resolves by `(hash,length)` from ANY pool folder, so `Test-PoolResolves` only fails once NO copy survives anywhere - "pool broken" already MEANS the bytes are gone, and nothing internal can repair that. (3) The real recovery path already exists and is not this: if the file still exists in the source, SR-053 treats a blank `DataPath` as damage being healed and the next ordinary backup re-stores it; if it exists in neither source nor pool, `-Action Verify` and both restorers already say so loudly (ContentMissing, exit 1). The only coherent scope left would be ingesting bytes from OUTSIDE the store (a second copy of the backup, a restored tree) - a different feature from what the name implies, never specified and not requested. Filed when Mirror-era storage made a row's bytes reachable at exactly one path; WP9 dissolved that premise, as it did S3's. WP5's DIAGNOSIS half (SR-049's R3/R4/R5 findings, which say exactly what is missing and where) is retained and remains useful. | **No further work. Retired, not deferred again.** The premise expired with Mirror: there is no state in which this could recover bytes that some other, already-working path could not. If ingesting bytes from a SECOND copy of a backup is ever wanted, file it as its own need with its own SN - it is a different feature, and it should not inherit this name. | RETIRED 2026-08-26 |
...
## D-1 / D-5 and the test-battery import — closed by WP9 (moved out of status.md 2026-08-25)

WP9 fixed D-1 and D-5 by **deleting their hazard class**: all storage is
content-addressed, the Mirror/`PreserveFolderTree` layout is gone, and
browsability is a generated view outside the backup root. Rows kept verbatim for
the record; the narrative is the WP9 G3 entry in status.md's audit log.

| Item | What (verified 2026-08-24) | Decision asked | State |
|---|---|---|---|
| **D-1 / D-5 — Mirror dedup design** | Mirror addressed data files by PATH while dedup handed that address to rows meaning "these exact bytes": an ordinary edit of one of two duplicate files destroyed the last copy (`Save-SupersededData`'s source-based survival test authorized the in-place overwrite), orphaning the borrower and every blank snapshot row — invisible to everything but `-Deep`, post-mortem. D-5 (same-run duplicates stored twice) was the same mechanism's other face. Hash-addressed mode was proven immune by repro. | **Pick the design** — (1) end cross-path sharing in Mirror; (2) copy-on-write/heal borrowers; (3) content-address all storage, Mirror as a restore view. | **CLOSED — option 3 shipped as WP9 (2026-08-25).** Mirror deleted (SR-058), owner election + intra-run memo (SR-060) fixed D-5, preservation retargeted to the FINAL manifest (SR-059/SR-051) fixed D-1's family, migration deleted and a legacy store refused (SR-061), browse view generated (SR-062), config v2 (SR-063), linear audit (SR-064). Kit unchanged at revision 6. |
| **Test-battery import** | The HomeHub drill assertion that caught D-1 (blank-row hashes cross-checked against the verified pool) had no FileBackup equivalent, and `G2.8 Dedup_singleDataPath` was VACUOUS (`-le 2` passed under D-5). | Approve as the test scope of the D-1..D-4 fix WPs. | **CLOSED — fully ported.** The kit-bump WP took the byte-verified orphan second pass (TC-116), nested dot-directories + Windows Hidden (TC-113/114), same-length corruption (TC-108/110) and the de-vacuumed `G2.8`. WP9 took the four that were impossible while Mirror existed: owner-edit and **borrower**-edit across `copies={2,3}` (TC-118), same-run duplicates (TC-119), and owner-or-borrower-removed-while-others-live (TC-135) — plus the SR-059 pool census and name-justification audit (TC-122). The drill SCRIPT itself was never pulled into this repo; only its assertions. |
| **`Copy-SourceFileToBackup` succeeds into a directory destination** (step-4 review n6) | If the destination path named an existing DIRECTORY, `Copy-Item` copied the file *into* it and the function reported success, so the manifest row named a directory. | None — found incidentally by the independent reviewer. | **CLOSED — fixed at WP9 step 8** (directory-destination guard in `Copy-SourceFileToBackup`, pinned by a Pester test). |

## Closed 2026-08-26 (WP10 and WP11)

Moved out of [status.md](status.md) when the live list was swept clean.
The narrative for each is in status.md's audit log under its date.

| Item | What / disposition | Decision asked | State |
|---|---|---|---|
