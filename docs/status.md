...
| Item | What | Decision asked | State |
|---|---|---|---|
| **One unreproduced empty-manifest run in the SR-057 Deny-ACE case** | `RestoreVerify.Tests.ps1`'s `one Deny-ACE hidden directory fails the set LOUDLY but still writes the manifest` failed ONCE in three Full runs on 2026-08-26: the manifest was written and the run exited 1 as designed, but held **zero** rows instead of the one reachable `good.txt`. **The first hypothesis - that `Get-DataFile`'s recursive walk abandons the whole enumeration on meeting a Deny ACE - was tested and DISPROVEN the same day.** Two harnesses (`denyrepro.ps1`, `denyfull.ps1`, scratch): 100 direct `Get-DataFile` walks, 40 idle and 60 under 8-way CPU load, returned all 4 reachable files and exactly 1 error record every time, with ZERO unreported shortfalls; 25 full-pipeline runs under 6-way load produced 0 failures. Re-ran clean besides: the test alone passes, the whole unit suite alone passes **425/425**. **Not introduced by WP10** - `Get-SourceDirectoryRecord`'s walk runs at step 12.5, AFTER the manifest is written. **Mechanism currently UNKNOWN.** | Two separable questions. (a) Diagnosis: leave it open and instrumented, since a retry would mask the evidence before the cause is known. (b) The real exposure this exposed, independent of the cause: a walk that comes back SHORT with the shortfall unreported would evict the missing rows as "removed from source". Zero files is already refused on an established store by the `AllowEmptySource` delete-all guard; **suspiciously fewer** is not. Generalising that guard from "zero" to "a large unexplained shortfall vs. the previous manifest" closes the family without needing the cause. **Driver recommends (b) as a small, separate WP; NOT a retry loop** - retry makes a loud failure rarer without making a silent shortfall impossible, which is the wrong trade for a backup tool. | **OPEN, unreproduced.** Consequence bounded today: on an established store the delete-all guard refuses before mutation; on a first backup nothing existed to lose; either way the run exits 1. |
