...
The table above is the hand-kept editorial overview; the detailed map below is
**generated from the modules' AST** by `scripts/gen_arch_map.ps1` (per-module
summary, internal cross-module dependencies, and every function with its
`Implements:` back-links). Do not edit it by hand — `check.ps1` fails if it is
stale. The same block is mirrored in [docs/architecture.md](docs/architecture.md).
