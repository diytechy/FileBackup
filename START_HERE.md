...
- **Must-have outcomes:** correct backup + **bit-exact restore from the backup
  folder alone**; dedup; content-addressed storage (±Compress); change-folder
  history; mail (optional). *(WP9 removed Mirror/`PreserveFolderTree`; the
  test matrix is Plain/Compress — see docs/status.md.)*
